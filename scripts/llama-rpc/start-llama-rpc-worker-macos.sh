...
if [[ "$device" == *,* ]]; then
    printf 'Exposing multiple devices (%s); the host tensor split decides the GPU/RAM ratio on this worker.\n' \
        "$device" >&2
fi

printf 'WARNING: RPC has no authentication or encryption. Use only on a trusted private network.\n' >&2
exec "$rpc_server_path" "${arguments[@]}"